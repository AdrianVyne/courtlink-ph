...
- [x] Implement proof upload to S3-compatible object storage, transaction references, staff approval/rejection, and short-lived signed review URLs (two-hour escalation still pending).
...
- [x] Add in-app notifications with a replaceable email adapter, emitted on court/coach proof, refund, and offer events (favorites, promotions, reviews, moderation, and PWA still pending).
